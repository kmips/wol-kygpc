...
if (!(appFeedback === '') && appFeedbackemail === '' ) {alert("App feedback is enabled but no email address is entered. Check out mydata.js.")}
...
if (localStorage.getItem('lastOpenedVersion') === null) {
  ...
  localStorage.setItem('lastKnownStateMainWin', JSON.stringify(currentStateMainWindow))
  localStorage.setItem('lastKnownStateSecWin', JSON.stringify(currentStateSecWindow))
} 

//Or if the last version opened is the same as this one, 
else if (JSON.parse(localStorage.getItem('lastOpenedVersion')) === thisAppVersion) {
  
  //get the stored values for panes' last known states into our session memory for use. 
  currentStateMainWindow = JSON.parse(localStorage.getItem('lastKnownStateMainWin'))
  currentStateSecWindow = JSON.parse(localStorage.getItem('lastKnownStateSecWin'))
} 
...
const sideBarClick = e => {
  ...
  if (e.currentTarget.dataset.windowName === 'mainWin') {
    ...
    var i = currentStateMainWindow.findIndex(obj => obj.folder === folderToSave)
    ...
    var i = currentStateMainWindow.findIndex(obj => obj.folder === e.currentTarget.dataset.folder)
    ...
    document.getElementById("mainFrame").src = `./${e.currentTarget.dataset.folder}/${currentStateMainWindow[i].fileToView}`;

    //To help the user keep track of where they are, set the collection name in the title bar
    remote.getCurrentWindow().setTitle(thisAppName + "   ||   " + e.currentTarget.dataset.collectionName) 
    ...
    var i = currentStateSecWindow.findIndex(obj => obj.folder === e.currentTarget.dataset.folder)
    document.getElementById("mainFrame").src = `./${e.currentTarget.dataset.folder}/${currentStateSecWindow[i].fileToView}`;
    remote.getCurrentWindow().setTitle(thisAppName + "   ||   " + e.currentTarget.dataset.collectionName) 
    localStorage.setItem('lastKnownStateSecWin', JSON.stringify(currentStateSecWindow))
  }
}
...
myData.collections.forEach(item => {
  ...
  if (item.horizontalLineFollows === false) {sidebarMenu.appendChild(document.createElement('br'))}
  ...
  if (remote.getGlobal('sharedObj').loadingMain === true) {
    document.getElementById("mainFrame").src = `./${currentStateMainWindow[0].folder}/${currentStateMainWindow[0].fileToView}`;
  } else if ((remote.getGlobal('sharedObj').loadingMain === false)) {
    document.getElementById("mainFrame").src = `./${currentStateSecWindow[0].folder}/${currentStateSecWindow[0].fileToView}`;
  }
}) 
...
if (remote.getGlobal('sharedObj').loadingMain === true) {
  ...
  if (!(appFeedback === '')) {
    ...
    sidebarMenuItem = document.createElement('a');
    sidebarMenuItem.setAttribute(`href`, `mailto:${appFeedbackemail}?subject=${appFeedbacksubject}`);
